...
class LightningDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            ...
            self.val_len = len(self.val_data.time)
        ...
        if stage == "test" or stage is None:
            self.test_data, self.test_transform, self.test_target_transform = get_xr_dataset(
                ...
                materialize=True,    # keep 0 workers here too
            )
            self.test_len = len(self.test_data.time)
    ...
    def train_dataloader(self):
        ...
        xr_dataset = DownscalingDataset(
            self.train_zarr_path,              # path only
            self.variables,
            self.target_variables,
            self.time_range,
            os.path.join(self.dataset_transform_dir,'input.pickle'),            # reuse fitted transforms
            os.path.join(self.dataset_transform_dir,'input.pickle'),
            self.train_len
        )
